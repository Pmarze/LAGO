...
void fun_lcd_num(int fd, int a){
	if(a==1){character(fd, (uint8_t *) N_1);}
	if(a==2){character(fd, (uint8_t *) N_2);}
	if(a==3){character(fd, (uint8_t *) N_3);}
	if(a==4){character(fd, (uint8_t *) N_4);}
	if(a==5){character(fd, (uint8_t *) N_5);}
	if(a==6){character(fd, (uint8_t *) N_6);}
	if(a==7){character(fd, (uint8_t *) N_7);}
	if(a==8){character(fd, (uint8_t *) N_8);}
	if(a==9){character(fd, (uint8_t *) N_9);}
	if(a==0){character(fd, (uint8_t *) N_0);}
}

void fun_println(int fd, char arr[]){
	int i;
	for(i=0 ; i<strlen(arr); i++){
		if(arr[i]==abecedary_upper[1]){character(fd, (uint8_t *) B_M);}
		if(arr[i]==abecedary_upper[2]){character(fd, (uint8_t *) C_M);}
		if(arr[i]==abecedary_upper[3]){character(fd, (uint8_t *) D_M);}
		if(arr[i]==abecedary_upper[4]){character(fd, (uint8_t *) E_M);}
		if(arr[i]==abecedary_upper[5]){character(fd, (uint8_t *) F_M);}
		if(arr[i]==abecedary_upper[6]){character(fd, (uint8_t *) G_M);}
		if(arr[i]==abecedary_upper[7]){character(fd, (uint8_t *) H_M);}
		if(arr[i]==abecedary_upper[8]){character(fd, (uint8_t *) I_M);}
		if(arr[i]==abecedary_upper[9]){character(fd, (uint8_t *) J_M);}
		if(arr[i]==abecedary_upper[10]){character(fd, (uint8_t *) K_M);}
		if(arr[i]==abecedary_upper[11]){character(fd, (uint8_t *) L_M);}
		if(arr[i]==abecedary_upper[12]){character(fd, (uint8_t *) M_M);}
		if(arr[i]==abecedary_upper[13]){character(fd, (uint8_t *) N_M);}
		if(arr[i]==abecedary_upper[14]){character(fd, (uint8_t *) O_M);}
		if(arr[i]==abecedary_upper[15]){character(fd, (uint8_t *) P_M);}
		if(arr[i]==abecedary_upper[16]){character(fd, (uint8_t *) Q_M);}
		if(arr[i]==abecedary_upper[17]){character(fd, (uint8_t *) R_M);}
		if(arr[i]==abecedary_upper[18]){character(fd, (uint8_t *) S_M);}
		if(arr[i]==abecedary_upper[19]){character(fd, (uint8_t *) T_M);}
		if(arr[i]==abecedary_upper[20]){character(fd, (uint8_t *) U_M);}
		if(arr[i]==abecedary_upper[21]){character(fd, (uint8_t *) V_M);}
		if(arr[i]==abecedary_upper[22]){character(fd, (uint8_t *) W_M);}
